...
namespace z1 {
	...
	VertexBuffer::Layout const SkeletalMesh::VertexData::s_layout = {
			{DataType::Float3},
			{DataType::Float3},
			{DataType::Float2},
			{DataType::Float2},
	...
	void StaticMesh::draw_primitive(
		...
		if (filter && !filter(mi ? mi->get_flags() : MaterialFlags::Default))
			return;
	...
	size_t SkeletalMesh::Primitive::get_triangle_count() const {
		auto vcount = m_vertex_array->get_element_count();
		switch (m_primitive_type) {
		case PrimitiveType::Points: return vcount;
		case PrimitiveType::LineStrip: return vcount - 1;
	...
	SkeletalMesh::SkeletalMesh(std::shared_ptr<Storage> const& storage)
		: m_bound_min(storage->bound_min)
		, m_bound_max(storage->bound_max) {
		auto vertex_buffer = VertexBuffer::create(
			storage->vertices.data(),
	...
	AssetMeta SkeletalMesh::Storage::import(Filepath const& path) const {
		BinaryFile bf{};

		auto vdata_size = vertices.size() * sizeof(SkeletalMesh::VertexData);
		auto idata_size = indices.size() * sizeof(uint32_t);
		...
		bf.set_yaml(yaml.c_str());
		if (!bf.save(file.concat(".bin"))) {
			CORE_ERROR("failed to save skeletal mesh storage: {0}", path.generic_string());
			return {};
		}
	...
	std::shared_ptr<SkeletalMesh> SkeletalMesh::load(Guid const& guid) {
		...
		YAML::Node node = YAML::Load(bf.get_yaml());
		auto storage = std::make_shared<SkeletalMesh::Storage>();
		...
		auto vdata_size = vertex_count * sizeof(SkeletalMesh::VertexData);
		auto idata_size = index_count * sizeof(uint32_t);

		auto vdata_slice = bf.get_data_slice(0, vdata_size);
		if (vdata_slice.size != vdata_size) {
			CORE_ERROR("corrupted skeletal mesh storage: {0}", file.generic_string());
			return nullptr;
		}
		std::memcpy(storage->vertices.data(), vdata_slice.ptr, vdata_slice.size);
