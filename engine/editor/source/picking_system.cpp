...
PickingSystem::PickingSystem(uint32_t w, uint32_t h) {
...

	{
		...
		desc.shader = g_runtime_context.m_asset_manager->get<Shader>(ENGINE_RESOURCE("shader/picking"));
...

	{
		...
		desc.blend = false;
		desc.cull_mode = CullMode::Back;
		desc.shader = g_runtime_context.m_asset_manager->get<Shader>(ENGINE_RESOURCE("shader/picking_sprite"));
