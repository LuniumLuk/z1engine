...
PY_ENUM_RE = re.compile(r'py::enum_<.*?>(?:\s*\(.*?\))?\s*\(\s*\w+\s*,\s*"(?P<name>\w+)"\)')
PY_ENUM_VAL_RE = re.compile(r'\.value\s*\(\s*"(?P<name>\w+)"')
...
def parse_manual_bindings(file_path):
	...
	for line in lines:
		...
		if current_enum:
			val_match = PY_ENUM_VAL_RE.search(line)
			if val_match:
				enums[current_enum].append(val_match.group('name'))
			continue
...
def generate_python_stubs(sorted_structs, sorted_enums, fields, struct_bodies, has_default_ctor, enum_items):
	...
	manual_classes, manual_enums, manual_functions = parse_manual_bindings(MANUAL_BINDINGS_FILE)
	...
	# Generate Manual Functions
	for func_name in manual_functions:
		if func_name == "register_event_listener":
			code.append(f"def {func_name}(type: EventType, callback: Any) -> None: ...")
		else:
			code.append(f"def {func_name}(*args: Any, **kwargs: Any) -> Any: ...")
	...
	# Generate Manual Enums
	for enum_name, values in manual_enums.items():
