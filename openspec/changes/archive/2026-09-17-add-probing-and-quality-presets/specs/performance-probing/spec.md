...
(no timing code, no GL calls, no logging) when it is not. The `Hybrid` build configuration SHALL define
`ENABLE_PROBING`. The existing `ENABLE_PROFILE` chrome-trace instrumentation SHALL remain independent;
probing is a separate, lightweight CPU+GPU budget facility.

#### Scenario: Probing disabled
- **WHEN** the project is built without `ENABLE_PROBING`
...
- **WHEN** the project is built with `ENABLE_PROBING` (Hybrid configuration)
