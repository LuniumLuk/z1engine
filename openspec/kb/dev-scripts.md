...
|---------|--------|-------------|
| `generate` | `commands/generate.py` | Regenerate project files via premake5 (add `--probing` to build the Hybrid config with the frame prober; add `--full-symbols` on macOS to keep full `-g` instead of line tables; add `--unity` to build engine projects from unity blobs) |
...
- `--unity` -- (generate) build engine projects from generated unity blob translation units (macOS + Windows); regenerate after adding/removing sources; wipe `engine/intermediate` before measuring (see `openspec/kb/build.md`)
