...
namespace z1 {
    ...
    double Timer::fixed_update_delta = 0.016;
    ...
    void Timer::update() {
        ...
        auto delta_time_count = std::chrono::duration<double, std::micro>(now - m_prev_timepoint).count();
    ...
    double Timer::get_total_time() const {
        auto total_time_count = std::chrono::duration<double, std::micro>(m_prev_timepoint - m_start_timepoint).count();
